...
- 2026-08-05: **the dashboard rework found a commit-path bug, not a layout one.** The lifecycle card was reading `state_counts(conn)` across *all* roots, and reported 140 already-filed library files as "found, not yet identified" — the same shape as the P17-03 Browse-count bug, and the second time counts have been taken over every item when the panel is about one root. Scoped to `root='inbox'`, with the library total stated separately. `state_counts` now takes an optional root.
- 2026-08-05: chasing why 140 library items sat in `discovered` **with live proposals whose destination equalled their own path** turned up the real fault. `mark_committed_proposals` lived in the web commit route, behind `if not summary.partial` — so one file edited since planning made the whole plan partial and left *every* proposal 'proposed', including those whose files had already moved. And `cli.py` never called it at all. On this machine 140 of 239 review rows were files already in the library, each proposing to move itself to where it already was; 59% of the queue was noise, and it is a fair part of why Review "felt clunky". The bookkeeping now happens in `executor._execute_op`, per op, right after the item row moves — so it fires for every caller and however the rest of the plan goes. Two regression tests, both verified to fail without the fix.
- 2026-08-05: dashboard hierarchy — one hero answering "does anything want me?" with the single action that follows, above six equal widgets rather than beside them. The `h1` moved into the hero (the nav already says which page this is, and repeating it cost the first 55px); the search box lost its hero sizing and its `autofocus`, which stole the keyboard on a page whose point is to be read. Disk rows are grouped by `st_dev`, so four configured roots on one volume are one row instead of four identical warnings, with a bar because length is judged pre-attentively and a percentage has to be read.
- 2026-08-05: the polling attributes moved onto the partial's own root element. They were on a wrapper in `dashboard.html`, and `hx-swap="outerHTML"` replaces the element it is on — so the first swap replaced the wrapper with markup carrying no `hx-trigger`, and the dashboard stopped refreshing after five seconds. The wrapper also duplicated `id="dashboard-stats"` on first paint. Both covered by `test_dashboard_keeps_polling_after_the_first_swap`.
