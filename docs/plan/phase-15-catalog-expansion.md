...
**Acceptance criteria:**
- [x] Episode fixture resolves via TVmaze when TMDB is disabled (test); both-enabled ordering documented and tested. Verified against the live API.
...
- 2026-07-23: **second dormant-catalog find — embedded audio tags.** `classify_music` was also called without its `tags` argument, so ID3/Vorbis metadata (keyless, offline, and the strongest music signal) was never read; every audio file fell to the 0.45 heuristic fallback, *below* the 0.8 confidence threshold, so music never even received a destination. ffprobe already extracted the tags — only the wiring was missing. Fixed, with an end-to-end container drill: `unknown-track-01.mp3` → `Music/Rock/Queen/A Night at the Opera/Bohemian Rhapsody.mp3` at 0.90. `tests/test_catalog_wiring.py` now guards this bug class by asserting the pipeline *calls* the lookups.
- 2026-07-23: **still unwired: AcoustID + MusicBrainz.** `classify_music`'s `acoustid_lookup` / `musicbrainz_lookup` remain test-only injections. They matter only for audio with *no* usable embedded tags (now the minority case). Implementing them needs an AcoustID client (fpcalc fingerprint + duration → `/v2/lookup`, `ACOUSTID_KEY`) and a MusicBrainz recording client (1 req/s, no key) — follow `tools/openlibrary.py`'s shape and gate on `catalog_enabled(conn, "acoustid")` / `"musicbrainz"`.
- 2026-08-04: **AcoustID + MusicBrainz now wired — the last dormant catalog is closed.** New `tools/acoustid.py` (fpcalc fingerprint + duration → `/v2/lookup`, returns `{score, recording_id}`) and `tools/musicbrainz.py` (recording MBID → artist/album/title/year), both injected from `classify/__init__.py` and gated on their `catalog.<slug>.enabled` toggles. `tests/test_catalog_wiring.py` covers the bug class the same way it does for TMDB: it asserts the pipeline *calls* fpcalc for tagless audio, and asserts it does **not** when the key is missing or the toggle is off (fingerprinting a whole inbox with nowhere to send the result is pure cost). Live drill: real MBID `b1a9c0e9…` → Queen / A Night at the Opera / Bohemian Rhapsody / 1975; a nonsense MBID returns `None`. AcoustID could **not** be live-verified — no `ACOUSTID_KEY` on this machine, and the adapter correctly returns `None` in that case. Re-run the live check once a key exists (same open item TMDB had).
- 2026-08-04: MusicBrainz's client deliberately carries **no internal throttle**. `classify_music._rate_limited_musicbrainz_lookup` already enforces `settings.mb_rate_limit` around every call; adding a second one would sleep twice per lookup. Documented in the module docstring — any *other* caller has to bring its own.
- 2026-08-04: when a recording has several releases, the **earliest dated one wins** (an undated release sorts last). Otherwise a track's album resolves to whichever greatest-hits compilation MusicBrainz happens to list first, which is a worse destination than the original record.
- 2026-08-05: **P15-03 done.** `tools/tvmaze.py` (`singlesearch/shows` → `shows/{id}/episodebynumber`), wired through a new `tvmaze_lookup` injection on `classify_video` and gated on `catalog.tvmaze.enabled`. Ordering when both TV catalogs are on: TMDB is asked first and its show name wins, because a key is a deliberate choice; TVmaze is asked *as well* for episodes, since it supplies the episode title TMDB's search endpoint does not — `S03E09.mkv` becomes `S03E09 - The Rains of Castamere.mkv`. Agreement between the two lifts episode confidence to 0.90, above either alone. A 404 on the episode never discards the show match: releases renumber episodes all the time. Live-verified (The Expanse S02E06 → *Paradigm Shift*; Breaking Bad S05E14 → *Ozymandias*).
