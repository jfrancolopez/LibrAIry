...
- 2026-08-05: **Review became cards.** A table row could not hold a proposal — path, destination, evidence, four actions, an edit form and a preview — and the preview was opening inside a 15rem cell, which is why an expanded row read as a stamp of picture surrounded by empty page. Confidence is a band down the left edge so scanning for the doubtful ones does not mean reading fifty scores.
- 2026-08-05: previews play. `FileResponse` answers Range requests, so scrubbing a 1.5GB film costs a 206 and not a download (verified against the real file). Only formats a browser handles natively are offered — MKV and AVI would need ffmpeg running for the length of the video, and the preview says so rather than rendering a player that shows black. `preload="none"`, or expanding a page of previews starts fetching every one.
- 2026-08-05: sorting fights grouping. Rows herded back into albums do not arrive in the order you asked for, so the default sort keeps groups and every explicit sort returns one flat list. Recorded because the obvious "just add ORDER BY" produces a page that silently ignores the control.
- 2026-08-05: **"Don't want it" means quarantine.** This is the one place a person most wants a delete button, and the invariant says no. Quarantine is the honest version of the wish — the file leaves the inbox and leaves Review, restorable indefinitely — and it is retargeted through the normal planner and executor rather than special-cased, so it stays hash-verified, journaled and undoable.
- 2026-08-05: **`.metric strong` was sizing every nested `<strong>` at 2.1rem**, the size of the page's own h1. `.metric` does two unrelated jobs — a one-big-number tile in Browse and a plain section wrapper in Health, Quarantine and the Settings catalog grid — and a descendant selector reached the whole subtree of the second kind. Bound to a direct child. `tests/test_theme.py` now fails on any `font-size` applied to a bare element through a descendant selector, and on any heading level missing from the type scale (h4 had no rule at all).
- 2026-08-05: **Search folded into Browse.** Two tabs answered one question and the split was the confusing kind: Browse showed the library, Search showed the library *and* the inbox *and* quarantine with no default, so the same word found different things depending on which tab you stood in — and the inbox dominates, because that is where the volume is. Scope now defaults to the library and names each option in plain words. `/search` redirects with its query string intact. The read-only guard on Browse had to be rewritten: it asserted there were no `<form>` tags at all, but the invariant was always "cannot change anything", so it now allows GET forms and forbids every writing verb.
- 2026-08-05: nav went from nine equal tabs to three plus More. Commit is a step rather than a place, so it appears only when something is approved and waiting, with the count on it.
- 2026-08-05: backup gained a per-category picker with live sizes from the index (never a filesystem walk — this renders on every Settings load). Storing "everything" as the empty string is deliberate: it keeps one meaning for the default, and a category added in a later release is then included rather than quietly missing from a backup someone believes is complete.
