...
| **Reattach drive** | the same registered drive is back, at a different mount point — say where. Its identity is its marker and its filesystem, so this changes only the location: the destination, its policies, its history and its divergence set all stay with it. Refused unless the drive at that location proves it is that drive. Not *Reconnect*, which is what plugging it in does, and not *Move*, which sounds like the files |
