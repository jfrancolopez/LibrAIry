...
| **M1-06** Pages that do not survive their tables | COMPLETE — Browse and Search at M1 close; Health carried, and **closed 2026-09-14 at 321 ms** by the recorded-observation pattern this item said it was waiting for |
...
* ~~**M1-06 PARTIAL** — Health at 1.8 s against "well under a second". The
  ...
  **CLOSED 2026-09-14 at 321 ms**, by the recorded-observation pattern this
  line predicted. See `docs/performance.md`.
...
* ~~**M1-06 PARTIAL**~~ **CLOSED 2026-09-14.** Health at 1.8 s at a million, against "well under a
...
| **Offline drive relocation** | DONE 2026-09-09 — the missing half of the M3-03 identity contract: a drive's identity is its marker and its filesystem, so a drive that comes back at another mount point keeps its destination, its policies, its history and its divergence set. `offline_drives.relocate` |
...
| **Final polish and docs** | DONE 2026-09-14 — the vocabulary audit found the Dashboard paraphrasing two pinned offline-drive phrases; the docs audit found the README calling the library read-only, a tool list missing poppler and rclone, and four shipped features with no user-facing prose at all. Screenshots retaken from the current UI, and `ui_check.py --shot` so they are of the thing they name rather than the top of its page |
| **NAS responsiveness under load** | **PRODUCTION VALIDATION REQUIRED** — not reachable from here, and not pretended otherwise: `/Volumes/UNRAID` is permission-denied and every Docker context is local. The procedure is now executable rather than described — `scripts/nas_validation.py`, [nas-validation.md](nas-validation.md) — with a `--observe` mode that is safe to run on production and a `--drive` mode that refuses the installation's own folders |
